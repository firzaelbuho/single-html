...
- start point index.html di folder ini (/prompt-manager)
...
- promp (generated) kategori ada : code, image, audio, video, text others (bedakan warna)
- bisa kasih tags ke prompt (opsional)
- ada fitur filter (by category, by tags) search (judul prompt, isi prompt) dan sort  (by date)
- bissa export dan import data prompt (single / multiple prompts) dengan format json
